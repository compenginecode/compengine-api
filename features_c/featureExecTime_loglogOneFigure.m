...
%         timeMat = [timeMat, C{3}(ismember(C{2}, wantedFeatureNames))];
    ...
    for featureInd = sortInds
       ...
       scatter(TSLength, timeMat(:,featureInd), ...
           'LineWidth', 2, ...
           'MarkerFaceColor', colors{mod(count, 6)+1}, ...
           'MarkerEdgeColor', colors{mod(count, 6)+1});
       hold on
       set(gca, 'xscale', 'log')
       set(gca, 'yscale', 'log')
       ...
       sp = plot(TSLength, exp(p(2))*TSLength.^p(1), ...
       'Color', colors{mod(count, 6)+1}, ...
       'LineStyle', linestyles{floor(count/6)+1}, ...
       'LineWidth', 2);
       ps = [ps, sp];
    ...
    legend(ps, featureNamesTemp(sortInds), 'Interpreter', 'none');

    % execution time for all features
    tsLengths = unique(TSLength);
    fprintf('\n');
    for tsLength = tsLengths'
       fprintf("N = %i, overall time = %1.3fms \n", tsLength, mean(sum(timeMat(TSLength==tsLength,:),2)));
    end
%     % errors
